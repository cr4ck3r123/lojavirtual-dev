...
function listarCasaCozinha($pagina, $itens_por_pagina) {
    $pdo = conecta();
    try {
        $listar = $pdo->query("SELECT * FROM smartphone LIMIT $pagina, $itens_por_pagina");
        $listar->execute();
...
function listarCasaCozinhaTotal() {
    $pdo = conecta();
    try {
        $listar = $pdo->query("SELECT * FROM smartphone");
        $listar->execute();
...
function recuperaDadosCasa($id) {
    $pdo = conecta();
    try {
        $pegaDados = $pdo->prepare("SELECT * FROM smartphone
WHERE id = ?");
